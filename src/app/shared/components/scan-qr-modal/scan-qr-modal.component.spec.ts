...
describe('ScanQrModalComponent', () => {
  ...
  beforeEach(() => {
    ...
    TestBed.configureTestingModule({
      ...
      imports: [IonicModule],
      providers: [Renderer2, { provide: ModalController, useValue: modalControllerMock }],
    }).compileComponents();
  ...
  it('should dismiss on unauthorized', async () => {
    const modalControllerSpy = spyOn(modalControllerMock, 'dismiss');
    const spy = spyOn(component.barcodeScanner, 'checkPermission');
    spy.and.returnValue(Promise.resolve({ granted: false }));
    fixture.detectChanges();
    await fixture.whenStable();
    expect(spy).toHaveBeenCalledTimes(1);
    expect(modalControllerSpy).toHaveBeenCalledWith(null, 'unauthorized');
  });
  ...
  it('should dismiss on close', async () => {
    ...
    await fixture.whenStable();
    expect(modalControllerSpy).toHaveBeenCalledWith(null, 'cancelled');
  });
});
