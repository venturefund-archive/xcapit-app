...
export class NativeTokenSend implements Send {
  ...
  static create(from: string, to: string, amount: number, coin: Coin, wallet?: Wallet) {
    const provider = new ERC20Provider(coin);
    const signer = wallet ? wallet : new VoidSigner(from);
    const canSignTx = wallet ? true : false;
    const _aToken = new NativeToken(new ERC20Provider(coin), signer);
    const _anAmount = { value: amount, token: coin.name };

    return new this(from, to, _anAmount, _aToken, canSignTx);
  }
