  ...
  template: `
    <ion-card class="ux-card ion-padding ion-no-margin cyc">
      ...
      <div class="cyc__cumulative-yields__token">
        ...
        <ion-text class="ux-font-text-xl" *ngIf="this.yield">
          {{ yield.value | formattedAmount }} <span class="ux-font-header-titulo">{{ yield.token }}</span>
        </ion-text>
      </div>
      <div class="cyc__cumulative-yields__usd">
        ...
        <ion-text class="ux-font-titulo-xs" *ngIf="this.yield"
          >= {{ usdYield.value | formattedAmount: 14:5 }} <span>{{ usdYield.token }}</span>
        </ion-text>
      </div>
    </ion-card>
  `,
