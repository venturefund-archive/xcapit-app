      ...
      <div class="ffp__content">
          ...
          >
            <div class="custom-button">
              <ion-icon
                ...
                [name]="this.status === 'active' ? 'ux-pause' : 'ux-resume'"
              ></ion-icon>
              <ion-label
                class="custom-button__label ux-font-lato ux-fweight-regular ux-fsize-14"
                color="uxsemidark"
              >
                {{
                  (this.status === 'active'
        ...
        <div class="ffp__content__right">
          <ion-button
            ...
            fill="clear"
            expand="full"
            [disabled]="!this.status || this.status === 'pausado'"
          >
            <div class="custom-button">
  ...
  pauseResumeFund() {
    if (this.status === 'active') {
      this.apiFunds.pauseFundRuns(this.fundName).subscribe((res) => {
        this.toastService.showToast({
          message: this.translate.instant(
            'funds.fund_finish_pause_fund_card.fund_paused'
          ),
        });
        this.status = 'pausado';
      });
      ...
      this.apiFunds.resumeFundRuns(this.fundName).subscribe((res) => {
        this.toastService.showToast({
          message: this.translate.instant(
            'funds.fund_finish_pause_fund_card.fund_resumed'
          ),
        });
        this.status = 'active';
      });
