    ...
    <ion-header>
      <ion-toolbar>
        <ion-buttons slot="start">
          <ion-back-button [defaultHref]="this.defaultBackRoute"></ion-back-button>
        </ion-buttons>
        ...
        <ion-card *ngFor="let run of fundRuns">
            ...
            <ion-grid>
              <ion-row justify-content-end>
                <ion-col align-self-stretch>
                  <ion-card-title
                    >{{ 'funds.fund_runs.card_title' | translate }} {{ run.id_corrida }}</ion-card-title
                  ></ion-col
                >
                <ion-col align-self-stretch>
                  <ion-button
          ...
          <ion-card-content>
            ...
            <ion-row>
              ...
              <ion-col
                ><ion-badge color="primary"> {{ run.estado | stateShowName }}</ion-badge></ion-col
              >
            </ion-row>
            <ion-row>
            ...
            <ion-item-divider *ngIf="run.porcentaje_ganancia"> </ion-item-divider>
            <ion-row *ngIf="run.porcentaje_ganancia">
              <ion-col>{{ 'funds.fund_runs.gain_current_currency_title' | translate }} {{ run.currency }}:</ion-col
              ><ion-col
                ><ion-badge color="{{ run.porcentaje_ganancia > 0 ? 'success' : 'danger' }}"
            ...
            <div *ngIf="run.estado === 'finalizado'">
              ...
              <ion-row *ngIf="run.inversion">
                <ion-col>{{ 'funds.fund_runs.inversion_title' | translate }} </ion-col
                ><ion-col>{{ run.inversion | number: '1.2-6' }} {{ run.currency }}</ion-col>
              </ion-row>
              <ion-row *ngIf="run.porcentaje_deposito">
                <ion-col>{{ 'funds.fund_runs.commission_percentage_title' | translate }} </ion-col
                ><ion-col>{{ run.porcentaje_deposito * 100 | number: '1.2-2' }} %</ion-col>
              </ion-row>
              <ion-row>
                <ion-col>
                  <span *ngIf="run.porcentaje_ganancia > 0">
                    {{ 'funds.fund_runs.commission_amount_title' | translate }}</span
                  >
                  <span *ngIf="run.porcentaje_ganancia <= 0">
...
export class FundRunsPage implements OnInit {
  ...
  private setDefaultBackRoute() {
    this.defaultBackRoute = this.selectedFund ? `/funds/fund-summary/${this.selectedFund}` : this.defaultBackRoute;
  }
