  ...
  template: `
    <ion-content class="ion-padding">
      <div *ngIf="!this.isValidEmail && !this.isValidating" class="ion-text-center">
        <h3>
          {{ 'users.email_validation.error_title' | translate }}
        </h3>
        <div class="ion-padding-top">
          {{ 'users.email_validation.error_text' | translate }}
        </div>
        <div class="auth-button ion-padding-top">
          ...
          >
            ...
            {{ 'users.email_validation.error_button' | translate }}
          </ion-button>
        </div>
        <div class="ion-text-left ion-padding-top">
          <ion-button fill="clear" size="small" type="button" routerDirection="back" [routerLink]="['/users/register']">
            {{ 'users.email_validation.register_link' | translate }}
          </ion-button>
        </div>
      </div>
    </ion-content>
...
export class EmailValidationPage implements OnInit, OnDestroy {
  ...
  async success(res) {
    this.toastService.showToast({
      message: this.translate.instant('users.register.success_text'),
    });
  }
